...
mod tests {
    ...
    fn test_default_manifest_parses() {
        ...
        assert_eq!(manifest.id.0, "com.orion.skills.web-search");
    ...
    fn test_tools_returns_web_search() {
        let tmp = std::env::temp_dir().join("orion_ws_skill_test");
    ...
    async fn test_execute_tool_blocked_query() {
        let tmp = std::env::temp_dir().join("orion_ws_skill_blocked");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
    ...
    async fn test_execute_tool_missing_key() {
        let tmp = std::env::temp_dir().join("orion_ws_skill_nokey");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
