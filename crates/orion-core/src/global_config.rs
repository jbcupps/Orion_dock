...
mod tests {
    ...
    fn test_global_config_roundtrip() {
        let tmp = std::env::temp_dir().join("orion_global_config_test");
    ...
    fn test_register_duplicate_fails() {
        let tmp = std::env::temp_dir().join("orion_global_config_dup");
        let _ = fs::remove_dir_all(&tmp);
    ...
    fn test_find_and_remove_agent() {
        let tmp = std::env::temp_dir().join("orion_global_config_find_remove");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
        let mut config = GlobalConfig::new(&tmp);
        config
