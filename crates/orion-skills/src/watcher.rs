...
mod tests {
    ...
    fn test_watcher_starts_on_existing_dir() {
        let tmp = std::env::temp_dir().join("orion_watcher_test_start");
    ...
    fn test_watcher_handles_nonexistent_dir() {
        let nonexistent = std::env::temp_dir().join("orion_watcher_nonexistent_12345");
    ...
    async fn test_watcher_detects_skill_toml_change() {
        let tmp = std::env::temp_dir().join("orion_watcher_test_detect");
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
